...
var Logger = function (opt, mockServices) {
    var opts = opt || {};
    mockServices = mockServices || {};
    if (mockServices.logger) {
        winston = mockServices.logger;
    }
    ...
    return {
        log: function () {
            ...
            return when.promise(function (resolve) {
                //console.log(level, isoDate() + ' [' + (opts.workerId || '') + ']' + ': ' + msg.join(' -> '), meta);
                resolve(winston.log(level, isoDate() + ' [' + (opts.workerId || '') + ']' + ': ' +
                    msg.join(' -> '), meta));
            });
        },
        ...
        err: function () {
            ...
            return when.promise(function (resolve) {
                resolve(winston.log('error', isoDate() + ' [' + (opts.workerId || '') + ']' + ': ' +
                    msg.join(' -> '), meta));
