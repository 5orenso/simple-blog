    ...
    function populate_article_sections (data) {
        ...
        for (var i in data_lines) {
            if (_.isString(data_lines[i])) {
                var data_line = data_lines[i].replace(/(^:([a-zA-Z0\-]+)\s*)(.*)$/, replacer_sections);
...
//            logger.log('replace_tags_with_content->replacer_tags->p2: ' + p2);
    ...
    function format_catlist (article, catlist) {
        ...
        article.tag_values.menu_onepage = '<ul class="catlist">';
...
//            console.log(art);
            ...
            article.tag_values['artlist-block'] += '<div class="artlist-art">' +
                (_.isArray(art.img) ? '<div class="artlist-image" style="background-image: url(\'/pho/' + art.img[0] + '?w=500\');"><a href="' + art.base_href + art.file + '"><img src="/images/pix.gif" style="height:100%; height:100%;"></a></div>' : '') +
