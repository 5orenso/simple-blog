                ...
                if (_.isObject(article.catlist)) {
                    for (var i in article.catlist) {
                        if (article.catlist[i]) {
                            ...
                            sitemap_xml += '    <url><loc>' + opts.protocol + '://' + opts.domain + '/' + cat.name + '/</loc><changefreq>weekly</changefreq><priority>0.85</priority></url>' + "\n";
                        }
                    }
                }
                if (_.isObject(article.artlist)) {
                    for (var j in article.artlist) {
                        if (article.artlist[j]) {
                            ...
                            sitemap_xml += '    <url><loc>' + opts.protocol + '://' + opts.domain + art.file + '</loc><changefreq>weekly</changefreq><priority>0.5</priority></url>' +  "\n";
                        }
                    }
                }
                sitemap_xml += '</urlset>' + "\n";
                fs.writeFile(app_path + 'template/current/sitemap.xml', sitemap_xml, function (err) {
                    if (err) {
                        reject(err);
                    }
                    resolve();
