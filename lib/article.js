...
//            logger.log('replace_tags_with_content->replacer_tags->p2: ' + p2);
    ...
    function format_artlist (article, artlist) {
        ...
        for (var i in artlist) {
            var art = artlist[i];
            article.tag_values.artlist += '<li><a href="' + art.file + '">' + art.title + '</a></li>';
            article.tag_values['artlist-block'] += '<div class="artlist-art">' +
                ...
                '<h3><a href="' + art.file + '">' + art.title + '</a></h3>' +
                '<p>' + (art.teaser || art.ingress || '') + '</p>' +
                '</div>';
        }
    ...
    function load_article (opt, callback) {
        ...
        logger.log('load_article->filename: ', opt.filename);
        fs.readFile(path.normalize(opt.filename), { encoding: 'utf8'}, function (err, data) {
            var article = populate_article_sections(data);
            article.file = opt.file;
            if (err) {
                logger.err(404, 'File not found: ' + opt.filename);
...
//            console.log(article);
    ...
    function load_catlist (root, callback) {
        ...
        var walker  = walk.walk('./content/articles', { followLinks: true });
        var catlist = [];
    ...
    function load_artlist (content_root, callback) {
        logger.log('load_artlist->root: ', root);
...
//            console.log(files);
    ...
    return {
        ...
        artlist: function (opt) {
            return when.promise( function (resolve, reject) {
                load_artlist(opts.content_path, function (err, artlist) {
        ...
        catlist: function () {
            return when.promise( function (resolve, reject) {
                load_catlist(opts.content_path, function (err, catlist) {
                    if (err) { reject({ error: err }); }
                    resolve({
                        catlist: catlist
                    });
