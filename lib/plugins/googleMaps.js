...
MyPlugin.prototype.replacer = function replacer(match, p1, p2) {
    var mapMode = 'search',  // place, directions, search, view, or streetview.
        help = false,
        error = [],
        usedOptions = [],
        optionalOptions = [],
        ...
        modeKeys = {
            place: {
                ...
                zoom: { required: false, type: 'string', values: '1-20', example: '10' },
                ...
                attribution_ios_deep_link_id: { required: false, type: 'string',
            ...
            directions: {
                ...
                mode: { required: false, type: 'string', values: 'driving, walking, bicycling, transit, flying',
                    example: 'walking' },
                avoid: { required: false, type: 'string', values: 'tolls, ferries, highways',
                    example: 'tolls|highways' },
                units: { required: false, type: 'string', values: 'metric, imperial', example: 'metric' }
            },
            search: {
                key: { required: true, type: 'string' },
                q: { required: true, type: 'string', example: 'Oslo,Norway or 69.9396,22.9232' },
            ...
            view: {
                ...
                zoom: { required: false, type: 'string', values: '1-20', example: '10' },
                maptype: { required: false, type: 'string', values: 'roadmap, satellite', example: 'satellite' },
            ...
            streetview: {
                ...
                heading: { required: false, type: 'string', values: '-180&deg; - 360&deg;', example: '270' },
                pitch: { required: false, type: 'string', values: '-90&deg; - 90&deg;', example: '-10' },
                fov: { required: false, type: 'string', values: '10&deg; - 100&deg;', example: '90' },
                language: { required: false, type: 'string' },
                region: { required: false, type: 'string' }
            }
        };
    // jscs:enable
    if (_.isString(p2)) {
        var queryString = '?' + p2.replace(/^;/, '');
        queryString = queryString.replace(/;/, '&');
        var parsed = url.parse(queryString, true);
    ...
    if (_.isObject(modeKeys[mapMode])) {
        ...
        for (var i in opt) {
            if (opt[i]) {
                ...
                if (_.isUndefined(modeKeys[mapMode][i])) {
                    excessiveOptions.push(i + '<pre><code class="hljs">' + JSON.stringify(opt[i], undefined, 2) +
                        '</code></pre>');
                    delete opt[i];
                }
            }
        ...
        for (var j in modeKeys[mapMode]) {
            if (_.isObject(modeKeys[mapMode][j])) {
                if (!modeKeys[mapMode][j].required && _.isUndefined(opt[j])) {
                    optionalOptions.push(j + ': <pre><code class="hljs">' + JSON.stringify(modeKeys[mapMode][j],
                        undefined, 2) + '</code></pre>');
                } else if (!_.isUndefined(opt[j])) {
                    usedOptions.push(j + ': <pre><code class="hljs">' + JSON.stringify(modeKeys[mapMode][j],
                        undefined, 2) + '</code></pre>');
                } else if (modeKeys[mapMode][j].required && _.isUndefined(opt[j])) {
                    error.push(j + ': <pre><code class="hljs">' + JSON.stringify(modeKeys[mapMode][j], undefined, 2) +
                        '</code></pre>');
//                    console.log(j, modeKeys[mapMode][j]);
    ...
    function writeHelpOptions() {
        return '<div>`' + match + '`<br>_Other optional options_:<ul>' +
            optionalOptions.map(function (el) {
                if (!el.required) {
                    return '<li>' + el + '</li>';
                }
            }).join('') +
            '</ul></div>' +
            '<div>`' + match + '`<br>_Used options_:<ul>' +
            usedOptions.map(function (el) {
                if (!el.required) {
                    return '<li>' + el + '</li>';
                }
            }).join('') +
    ...
    if (error.length > 0) {
//        console.log(error);
        return '<div><strong>ERROR with googleMaps plugin! `' + match + '` <br>Error: Missing parameters:</strong><ul>' +
            error.map(function (el) {
                return '<li>' + el + '</li>';
            }).join('') +
            '</ul>' +
            'Warn: Excessive options:<ul>' +
            excessiveOptions.map(function (el) {
                return '<li>' + el + '</li>';
            }).join('') +
            '</ul>' +
