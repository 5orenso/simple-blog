...
class CollectionModel {
    ...
    async update(dataObject) {
        return new Promise((resolve, reject) => {
            if (!dataObject.id) {
                return reject(new Error('No id given!'));
            }
            const query = util.cleanObject({ id: dataObject.id });
            ...
            if (dataObject.addEvent) {
                $push.events = {
                    date: new Date(),
                    message: dataObject.addEvent,
                };
                delete dataObject.addEvent;
            }

            const updateObject = {};
            if (util.isArrayWithContent(Object.keys($set))) {
                updateObject.$set = { ...$set };
            }
            if (util.isArrayWithContent(Object.keys($push))) {
                updateObject.$push = { ...$push };
            }
            this.Model.collection.updateOne(query, updateObject, (err, object) => {
                if (err) {
                    // console.error('CollectionModel.update.error', err);
                    return resolve(err);
                }
