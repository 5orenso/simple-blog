...
class ImageUtilities {
    ...
    async loadModels() {
        ...
        await faceapi.nets.ssdMobilenetv1.loadFromDisk('./lib/models');
        await faceapi.nets.faceLandmark68Net.loadFromDisk('./lib/models');
        await faceapi.nets.faceRecognitionNet.loadFromDisk('./lib/models');
        await faceapi.nets.faceExpressionNet.loadFromDisk('./lib/models');
    }
    ...
    async read($filename, disableAi = false, opt) {
        ...
        if (filename.match(/(jpg|jpeg)/i)) {
            ...
            if (this.image.exif && this.image.exif.lat && this.image.exif.lng) {
                if (opt.img && !opt.img.geo) {
