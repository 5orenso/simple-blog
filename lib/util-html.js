    ...
    function replaceMarked(input) {
        if (typeof input === 'string') {
            ...
            if (results && results.length > 0) {
                ...
                let prevLevel = 0;
                tocHtml.push('<ul class="list-unstyled ml-4">');
                for (let i = 0, l = tocList.length; i < l; i += 1) {
                    ...
                    tocHtml.push(`<li><a href="#${obj.anchor}">${obj.title}</a></li>`);
                    prevLevel = obj.level;
                }
            ...
            parsedMarkdown = parsedMarkdown.replace(/<!--toc-->/, `<div class='mb-4'>${tocHtml.join('\n')}</div>`);
