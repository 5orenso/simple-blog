...
var ArticleUtil = function (options, mock_services) {
    ...
    return {
        ...
        populateArticleSections: function populateArticleSections (data) {
            ...
            for (var i in data_lines) {
                if (_.isString(data_lines[i])) {
                    var line_contains_mode = false;
                    // var line_mode_equals_last_mode = false;
                    if (data_lines[i].match(/(^:([a-zA-Z0-9\-]+)\s*)(.*)$/)) {
                        line_contains_mode = true;
                    }
                    ...
                    if (_.isString(article[mode]) && article[mode].length > 0) {
                        ...
                        if (mode.match(/^(col|col\d+)$/)) {
                            ...
                            if (_.isEmpty(article[mode][colNumber[mode]])) {
                                article[mode][colNumber[mode]] = data_line;
                            } else {
                                article[mode][colNumber[mode]] += "\n" + data_line;
                            }                                                        
                        } else if (mode.match(/^(img|img2|img3|img4|img5|img6|img7|img8|img9|tag|images)$/)) {
        ...
        formatArticleSections: function formatArticleSections (article) {
            function replacer_wsd_style(match, p1, p2) {
                if (logger) {
                    logger.log('replaceTagsWithContent->replacer_wsd_style->p1,p2: ', p1, p2);
                }
                return '<img src="/pho/wsd/?data=' + encodeURIComponent(p1) + '">';
            }
            function replacer_flot(match, p1) {
                var xaxis = {};
                var yaxis = {};
            ...
            for (var field in article) {
                if (_.isString(article[field])) {
                    ...
                    } else if (field.match(/^(body|aside|ingress|teaser|footnote|col)/)) {
                        // Parse markdown to html
                        article[field] = article[field].replace(/```wsd\s+([\s\S]+?)```/g, replacer_wsd_style);
                        article[field] = article[field].replace(/```flot\s+([\s\S]+?)```/g, replacer_flot);
                    ...
                    if (field.match(/^(body|aside|ingress|teaser|footnote|col)/)) {
                        for (var i in article[field]) {
                            if (_.isString(article[field][i])) {
                                // Parse markdown to html
                                article[field][i] = article[field][i].replace(/```wsd\s+([\s\S]+?)```/g, replacer_wsd_style);
                                article[field][i] = article[field][i].replace(/```flot\s+([\s\S]+?)```/g, replacer_flot);
        ...
        replaceTagsWithContent: function replaceTagsWithContent (article) {
            function replacer_tags(match, p1, p2) {
                if (logger) {
                ...
                return article.tag_values[p1] || article[p1] || '';
            }
            function replacer_code_style(match, p1, p2) {
                if (logger) {
                    logger.log('replaceTagsWithContent->replacer_code_style->p1,p2: ', p1, p2);
                }
                return '<span class="comment">' + p1 + p2 + '</span>';
            }
            function replacer_hash_tags(match, p1, p2) {
                if (logger) {
            ...
            function replacer_users(match, p1, p2) {
                if (logger) {
            ...
            for (var i in fields) {
                if (fields[i]) {
                    ...
                    for(var j = 0; j < 10; j++){
                        ...
                        } else if (_.isArray(article[replace_field])) {
                            for (var k in article[replace_field]) {
                                if (_.isString(article[replace_field][k])) {
                                    article[replace_field][k] = article[replace_field][k].replace(/\[:([a-z_\-0-9]+)(\s[a-z_\-0-9]+)*?\]/g, replacer_tags);
                                    article[replace_field][k] = article[replace_field][k].replace(/\s+(#)([a-z_\-0-9\.]+)/gi, replacer_hash_tags);
                                    article[replace_field][k] = article[replace_field][k].replace(/\s+(@)([a-z_\-0-9\.]+)/gi, replacer_users);
                                    article[replace_field][k] = article[replace_field][k].replace(/^(\/\/\s+)(.+?)$/gm, replacer_code_style);
                                }
                            }
                        } else if (_.isString(article.tag_values[replace_field])) {
                            article.tag_values[replace_field] = article.tag_values[replace_field].replace(/\[:([a-z_\-0-9]+)(\s[a-z_\-0-9]+)*?\]/gm, replacer_tags);
                        }
                    }
                }
            }           
