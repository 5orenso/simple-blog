...
renderer.image = function image($href, title, text) {
    const src = $href;
    const href = $href.replace(/(w=[0-9]+)/, 'w=2000');
    return `<p class="image_inline"><a href="${href}" data-smoothzoom="group1" title="${title || text
    }"><img src="${src}" alt="${text}" title="${title || text}">` +
...
ArticleUtil.prototype.inlineImageSize = function inlineImageSize($input, $size) {
    const input = $input.replace(/\?w=[0-9]+/g, `?w=${$size}`);
    // const input = $input;
    return input;
};
...
ArticleUtil.prototype.populateArticleSections = function populateArticleSections($data) {
    ...
    // console.log(data);
    let yamlObj = {};
    const data = $data.replace(/^---([\s\S]+)---/, (match, p1) => {
        if (p1) {
            try {
