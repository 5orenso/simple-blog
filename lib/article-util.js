        ...
        fs.readdirSync(opt.path).forEach(function (file) {
            if (file.match(/\.js$/)) {
                try {
                    ...
                    if (_.isFunction(plugin.replacer)) {
                        //console.log('adding metrics hook to: ', file);
                        plugin.replacer = metrics.hook(plugin.replacer,
                            'simpleblog.articleUtil.plugins.' + file);
                    }
        ...
        fs.readdirSync(opt.path).forEach(function (file) {
            if (file.match(/\.js$/)) {
                try {
                    ...
                    if (_.isFunction(replacer.replacer)) {
                        replacer.replacer = metrics.hook(replacer.replacer,
                            'simpleblog.articleUtil.replacer.' + file);
                    }
...
renderer.image = function (href, title, text) {
    return '<p class="image_inline"><a href="' + href + '" data-smoothzoom="group1">' +
        '<img src="' + href + '" alt="' + text + '" title="' + (title ? title : text) + '">' +
        '</a></p>';
};

var ArticleUtil = function (options, mockServices) {
    var opts = options || {};
    ...
    return {
        ...
        getArticlePathRelative: function getArticlePathRelative(completePath, contentPath) {
                    ...
                    if (_.isString(article[mode]) && article[mode].length > 0) {
                        ...
                        if (mode.match(/^(col|col\d+)$/)) {
                            ...
                            if (mode.match(/img/)) {
                                ...
                                article[mode + '_text'].push(dataArray[1]);
                            } else {
