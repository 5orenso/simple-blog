...
loadPlugins({
    path: path.normalize(app_path + 'lib/plugins/')
});
loadReplacers({
    path: path.normalize(app_path + 'lib/replacers/')
});
                ...
                if (fields[i]) {
                    ...
                    for(var j = 0; j < 10; j++){
                        ...
                        if (_.isString(article[replace_field])) {
                            for (var i in myReplacers) {
                               var replacer = myReplacers[i];
                            ...
                            for (var k in article[replace_field]) {
                                if (_.isString(article[replace_field][k])) {
                                    for (var i in myReplacers) {
                                        var replacer = myReplacers[i];
                                        article[replace_field][k] = article[replace_field][k].replace(replacer.get('regexp'), replacer.replacer);
                                    }
