...
function returnString($obj, ...$args) {
    ...
    if (Array.isArray(obj)) {
        return utilHtml.replaceMarked(obj.map(o => `* ${o}`).join('\n'));
    }
    return obj;
}
...
ArticleUtil.prototype.dropFirstLetterAfterHr = function dropFirstLetterAfterHr($string) {
    const stringParts = $string.split('<hr>');
    for (let i = 0, l = stringParts.length; i < l; i += 1) {
        stringParts[i] = ArticleUtil.prototype.dropFirstLetter(stringParts[i]);
    }
    return stringParts.join('<hr>');
};

ArticleUtil.prototype.dropFirstLetter = function dropFirstLetter($string) {
    let mode = 0;
    ...
    for (let i = 0, l = articleKeys.length; i < l; i += 1) {
        ...
        if (_.isString(article[section])) {
            ...
            if (section.match(/^(title|author|published)/)) {
                ...
                article[section] = utilHtml.replaceMarked(article[section]);
            ...
            if (section.match(/^(body|aside|ingress|teaser|footnote|col|youtube)/)) {
                ...
                for (let i2 = 0, l2 = articleSectionKeys.length; i2 < l2; i2 += 1) {
                    ...
                    if (_.isString(article[section][key])) {
                        ...
                        article[section][key] = utilHtml.replaceMarked(article[section][key]);
                        this.buildTableOfContents(article, section);
                    }
                }
            }
...
ArticleUtil.prototype.formatArtlist = function formatArtlist($article, artlist, keepArtlist = false) {
    const article = $article;
...
ArticleUtil.prototype.formatArtAlllist = function formatArtAlllist($article, artlistall, keepArtlist = false) {
    const article = $article;
    if (!_.isObject(article.tagValues)) {
        article.tagValues = {};
    }
