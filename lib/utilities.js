...
function utilities() {
    ...
    function match(string = '', expression, options = 'i') {
        const regexp = new RegExp(expression, options);
    ...
    function removeTags(text) {
        if (tc.isString(text)) {
            // Remove script and all content
            const textWithoutWidgets = text.replace(/{{.+?}}/im, '');
            const textWithoutScript = textWithoutWidgets.replace(/<script.+?<\/script>/gim, '');
            const data = striptags(textWithoutScript);
            return data;
        }
        return text;
    }
    ...
    function termsCount(terms, $data) {
        ...
        if (typeof data === 'string') {
            data = tokenizer.tokenize(removeTags(data));
        }
    ...
    function termsRatio(terms, $content) {
        const tokenizer = new natural.AggressiveTokenizerNo();
        const content = removeTags($content);
        const totalWords = tokenizer.tokenize(content).length;
        const count = termsCount(terms, content);
        const words = Object.keys(count);
        for (let i = 0, l = words.length; i < l; i += 1) {
            const word = words[i];
            count[word] = count[word] / totalWords;
        }
        return count;
    }
    ...
    function classifyArticle(artlist = [], article) {
        ...
        for (let i = 0, l = artlist.length; i < l; i += 1) {
            const art = artlist[i];
            // classifier.addDocument(art.body);
    ...
    function tokenizeAndStem($sentence = '') {
        const sentence = removeTags(removeMarkdown($sentence.toLowerCase()));
    ...
    function tokenize($sentence = '') {
        const sentence = removeTags(removeMarkdown($sentence.toLowerCase()));
        const tokenizer = new natural.AggressiveTokenizerNo();
        const tokens = tokenizer.tokenize(sentence).filter(w => w.length >= 4);
        const distinctTokens = [...new Set(tokens)];
        return sw.removeStopwords(distinctTokens, sw.no);
    }
    ...
    function getWords(article) {
        ...
        // const ingressWords = tokenize(article.ingress);
        const metaDescriptionWords = tokenize(article.metaDescription);
        // const bodyWords = tokenize(article.body);
        // console.log(tokenize(article.body));
        const allWords = [titleWords, metaDescriptionWords].flat();
        allWords.forEach(w => words.add(w));
        return Array.from(words);
    }

    function maxLength(string, max, link = '') {
        if (typeof string === 'string') {
            const lngth = string.length;
            if (lngth > max) {
    ...
    function getShortText($sentence = '') {
        let sentence = removeLineBreaks($sentence);
        sentence = removeTags(sentence);
        sentence = removeMarkdown(sentence);
        // console.log({ sentence });
        return maxLength(sentence, 160);
    }
    ...
    function replaceLanguage(object, language) {
        const lang = language.charAt(0).toUpperCase() + language.slice(1);
        if (tc.isObject(object)) {
            object.title = object[`title${lang}`] ? object[`title${lang}`] : object.title;
            object.teaser = object[`teaser${lang}`] ? object[`teaser${lang}`] : object.teaser;
            object.ingress = object[`ingress${lang}`] ? object[`ingress${lang}`] : object.ingress;
            object.body = object[`body${lang}`] ? object[`body${lang}`] : object.body;
            object.dropdown = object[`dropdown${lang}`] ? object[`dropdown${lang}`] : object.dropdown;
            object.footerCol1 = object[`footerCol1${lang}`] ? object[`footerCol1${lang}`] : object.footerCol1;
            object.footerCol2 = object[`footerCol2${lang}`] ? object[`footerCol2${lang}`] : object.footerCol2;
            object.footerCol3 = object[`footerCol3${lang}`] ? object[`footerCol3${lang}`] : object.footerCol3;
            return object;
        }
        return object;
    }

    function useLanguage(data, language) {
        if (!language || language === 'no') {
            return data;
        }
        if (tc.isArray(data)) {
            return data.map(obj => replaceLanguage(obj, language));
        }
        if (tc.isObject(data)) {
            return replaceLanguage(data, language);
        }
        return data;
    }
    ...
    function wideDynamic(idx) {
        return idx % 2 === 0 ? 'float-left' : 'float-right';
    }

    function wideDynamicOther(idx) {
        return idx % 2 === 0 ? 'float-right' : 'float-left';
    }
